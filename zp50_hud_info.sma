...
public ShowHUD(taskid)
{
	...
	if (player != ID_SHOWHUD)
	{
		...
		if (LibraryExists(LIBRARY_AMMOPACKS, LibType_Library))
			ShowSyncHudMsg(ID_SHOWHUD, g_MsgSync, "%L: %s^nHP: %d - %L %s - %L %d^nLEVEL: %d - EXP (%d/%d)", 
				ID_SHOWHUD, "SPECTATING", player_name, 
				get_user_health(player), ID_SHOWHUD, "CLASS_CLASS", class_name, ID_SHOWHUD, "AMMO_PACKS1", zp_ammopacks_get(player),
				level, zp_level_get_exp(player), zp_level_get_req_exp(level));
		else
			ShowSyncHudMsg(ID_SHOWHUD, g_MsgSync, "%L: %s^nHP: %d - %L %s - %L $ %d", 
				ID_SHOWHUD, "SPECTATING", player_name, 
				get_user_health(player), ID_SHOWHUD, "CLASS_CLASS", class_name, ID_SHOWHUD, "MONEY1", cs_get_user_money(player),
				level, zp_level_get_exp(player), zp_level_get_req_exp(level));
	}
	else
	{
		new level = zp_level_get(ID_SHOWHUD);

		// Show health, class
		set_hudmessage(red, green, blue, HUD_STATS_X, HUD_STATS_Y, 0, 6.0, 1.1, 0.0, 0.0, -1)
		
		if (LibraryExists(LIBRARY_AMMOPACKS, LibType_Library))
			ShowSyncHudMsg(ID_SHOWHUD, g_MsgSync, "HP: %d - %L %s - %L %d^nLEVEL: %d - EXP (%d/%d)", 
				get_user_health(ID_SHOWHUD), ID_SHOWHUD, "CLASS_CLASS", class_name, ID_SHOWHUD, "AMMO_PACKS1", zp_ammopacks_get(ID_SHOWHUD),
				level, zp_level_get_exp(ID_SHOWHUD), zp_level_get_req_exp(level));
		else
			ShowSyncHudMsg(ID_SHOWHUD, g_MsgSync, "HP: %d - %L %s^nLEVEL: %d - EXP (%d/%d)", 
				get_user_health(ID_SHOWHUD), ID_SHOWHUD, "CLASS_CLASS", class_name,
				level, zp_level_get_exp(ID_SHOWHUD), zp_level_get_req_exp(level));
	}
}
